...
function map($url) {
	...
	$def_protocol = substr($url, 0, 8) === 'https://' ? 'https:' : 'http:';
	...
	if ($response['code'] === 200){
		...
		foreach ($links as $link) {
			...
			if (strtolower($rel) === 'nofollow') continue;
			...
			if (substr($href, 0, 3) === 'ftp:') continue;
			...
			if (in_array($href, $queue) || isset($pages[$href])) continue;
			...
			$addtoqueue = false;
			if ($kod) {
				if ($domx == $domain) $addtoqueue = true;
			} else {
				$addtoqueue = true;
			}

			if ($addtoqueue) $queue[] = $href;
...
if ($newmap === false) echo '    <span class="red">Cannot be indexed.</span>';
else $pages[$url] =& $newmap;
...
echo '</pre><textarea style="display:block;height:750px;width:60%;">',
	serialize($pages),
	'</textarea>';
